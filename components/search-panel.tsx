...
export default function SearchPanel() {
  const [activeTab, setActiveTab] = useState('model-search')
  ...
  const [salonMaleStaff, setSalonMaleStaff] = useState(false)
  const [salonFemaleStaff, setSalonFemaleStaff] = useState(false)
  const [salonCreditCardOk, setSalonCreditCardOk] = useState(false)
  const [salonMaxPrice, setSalonMaxPrice] = useState('50000')
      ...
      <div className="max-w-4xl mx-auto -mt-8 md:-mt-12">
        <div className="bg-white rounded-2xl shadow-lg p-5 md:p-6 border border-border">
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            ...
            {/* Model Search Tab */}
            <TabsContent value="model-search" className="space-y-4 mt-4">
              {/* Category Pills */}
                  ...
                  <button
                    ...
                    className={`px-4 py-1 rounded-full text-sm font-medium transition-all ${
                      cat.active
                        ...
                        : 'bg-neutral-soft text-muted-foreground hover:bg-border'
                    }`}
                  >
                    {cat.label}
                  </button>
              ...
              <div className="grid grid-cols-1 md:grid-cols-[minmax(0,2fr)_minmax(0,2fr)_auto] gap-3 items-end mt-4">
                ...
                <button onClick={() => setShowAdvancedFilters(!showAdvancedFilters)} className="text-primary hover:text-secondary text-sm md:text-base font-medium flex items-center gap-1 hover:gap-1.5 transition-all">
              ...
              {showAdvancedFilters && (
                <div className="bg-neutral-soft p-4 rounded-lg space-y-4 border border-border">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-4">
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" checked={femaleStaff} onChange={(e) => setFemaleStaff(e.target.checked)} className="w-4 h-4 rounded border-border" />
                        <span className="text-sm text-foreground">女性スタッフ</span>
                      </label>
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" checked={maleStaff} onChange={(e) => setMaleStaff(e.target.checked)} className="w-4 h-4 rounded border-border" />
                  ...
                  <div className="space-y-2">
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input type="checkbox" checked={creditCardOk} onChange={(e) => setCreditCardOk(e.target.checked)} className="w-4 h-4 rounded border-border" />
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      <select value={maxPrice} onChange={(e) => setMaxPrice(e.target.value)} className="w-full px-4 py-2.5 rounded-lg border border-border bg-white text-foreground text-sm appearance-none cursor-pointer focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2">
                        <option value="10000">¥10,000以下</option>
                        <option value="20000">¥20,000以下</option>
                        <option value="30000">¥30,000以下</option>
                        <option value="50000">¥50,000以下</option>
              ...
              <div className="flex justify-end pt-2">
                <Link href={`/search/models?area=&date=`}>
            ...
            <TabsContent value="salon-search" className="space-y-4 mt-4">
              {/* Category Pills */}
              <div className="flex flex-wrap gap-2">
                {categories.map((cat) => (
                  <button
              ...
              {showAdvancedFilters && (
                <div className="bg-neutral-soft p-4 rounded-lg space-y-4 border border-border">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-4">
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" checked={salonFemaleStaff} onChange={(e) => setSalonFemaleStaff(e.target.checked)} className="w-4 h-4 rounded border-border" />
                        <span className="text-sm text-foreground">女性スタッフ</span>
                      </label>
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" checked={salonMaleStaff} onChange={(e) => setSalonMaleStaff(e.target.checked)} className="w-4 h-4 rounded border-border" />
                        <span className="text-sm text-foreground">男性スタッフ</span>
                      </label>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input type="checkbox" checked={salonCreditCardOk} onChange={(e) => setSalonCreditCardOk(e.target.checked)} className="w-4 h-4 rounded border-border" />
                      <span className="text-sm font-medium text-foreground">クレジットカード決済OK</span>
                    </label>
                  </div>
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-foreground block">料金上限</label>
                    <div className="relative">
                      <select value={salonMaxPrice} onChange={(e) => setSalonMaxPrice(e.target.value)} className="w-full px-4 py-2.5 rounded-lg border border-border bg-white text-foreground text-sm appearance-none cursor-pointer focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2">
                        <option value="10000">¥10,000以下</option>
                        <option value="20000">¥20,000以下</option>
                        <option value="30000">¥30,000以下</option>
                        <option value="50000">¥50,000以下</option>
              ...
              {/* Submit Button */}
              <div className="flex justify-end pt-2">
                <Link href={`/search/salons?age=&hair=&area=`}>
                  <Button className="w-full md:w-auto bg-primary hover:bg-primary/90 text-primary-foreground rounded-lg px-6">
                    モデルを検索
                  </Button>
                </Link>
